function out = runix(varargin)

% RUNIX  Search in Files for specific string
% 
% RUNIX(Command,Pfad,'-r','-l','-v','-d','-n','-K#','.ext1','.ext2', ... )
% RUNIX(Command,File,'-r','-l','-v','-d','-n','-K#','.ext1','.ext2', ... )
%
%  '-r'  ==  recurse
...
opt = opt([ ( 2-follink : end-2 ) (end-1)*(1:nodispl) end*(1:verbose) ]);
