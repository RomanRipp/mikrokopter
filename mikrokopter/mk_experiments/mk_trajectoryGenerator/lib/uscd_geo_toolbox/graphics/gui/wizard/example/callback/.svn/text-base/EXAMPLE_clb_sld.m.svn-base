function par_sdl(par,group,action,clb,varargin)

% PAR_SLD  CallBack for SLIDER of Konfiguration-Parameter-Section
...
hs = getfield( ch , action               );  % Slider
he = getfield( ch , ['Edit' action(end) ]);  % Edit


val  = get( hs , 'value'    );
form = get( hs , 'userdata' );

[val,str,msg1,msg2] = val2str(val,form);
