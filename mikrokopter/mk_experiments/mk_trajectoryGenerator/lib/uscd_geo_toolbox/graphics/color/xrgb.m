function  [x,y,z] = xrgb(name)

% XRGB  returns X-Windows RGB-Colors 
%
% C = XRGB returns a 3-Column CellArray: 
%     { ColorName   HexString   UINT8_Color }
%        
%     example:  'white'   '#FFFFFF'   [ 255 255 255 ] 
%
% [ N , C , H ] = XRGB  returns Name N, ColorMap C, HexColors H
% 
%     example:   'white'   [ 1  1  1 ]  'FFFFFF'
%
% XRG( X ) with X ~=  0  returns unique Colors only.
% XRG( X ) with X <   0  returns unique Colors excluding Gray-Colors.
% XRG( X ) with X == NaN returns unique Gray-Colors only.
%
%
% [ C , H , N ] = XRGB( ColorName ) returns Color and HexColor
%
%     example:   [ 1  1  1 ]  'FFFFFF'   'white'
%
%
% The XRGB-Colors are defined in the SubFunction XRGB_COLORS
...
if is_name
   s = size(name);
   p = prod(s);
   is_name = ( ischar(name) & ( p == s(2) ) );
   if ~is_name
       if ~( isnumeric(name) & ( p == 1 ) )
           error('Input must be a String or single Numeric.');
       end
       is_modf = sign(name);       
   end
end
...
c = xrgb_colors;  % { Name HEX RGB }

%************************************************************
% [ C , H , N ] = XRGB( ColorName ) 
...
if is_name
   ...
   if ~any(ok)
       name = lower(name);
       ok = ( strcmp( name , lower(c(:,1)) ) );
       if ~any(ok)
           ok = ( strcmp( name , lower(strrep(c(:,1),' ','')) ) );
       end
   end 
...
if ~( is_modf == 0 )
   ...
   % Length of Blanks at End
   z = cumprod( double( y(:,end:-1:1) == 32 ) , 2 );
   z = sum(z,2);
   ...
   % Longest First
   x = 1 - ( x / size(y,2) ) / 10;  % [ 0 .. 0.1 ]

   %-------------------------------------------------
   % Check with Blanks inside Name and UpperCase

   x = x + 0.2 * ( sum((y==32), 2 ) > z ) + ...
           0.2 * ( y(:,1) == upper(y(:,1)) );

   %-------------------------------------------------
   % Sort by checked Length
   ...
   if ( is_modf == 1 ) | isnan(is_modf)
       ...
       y(find(x==000),1) = {'black'};
       y(find(x==255),1) = {'white'};
